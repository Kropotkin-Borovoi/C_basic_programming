...
void test (Matrix m1, Matrix m2) {
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			...
			if(fabs(Mas2_reverse.Mas[i * m1.N + j] - Mas2_reverse_.Mas[i * m1.N + j]) > pow(10, -4)) {
				printf ("\nMas2_reverse.Mas[%d] == %f\nMas2_reverse_.Mas[%d] == %f", i * m1.N + j, Mas2_reverse.Mas[i * m1.N + j], i * m1.N + j, Mas2_reverse_.Mas[i * m1.N + j]);
				printf("error");
				exit(1);
			}
		}
